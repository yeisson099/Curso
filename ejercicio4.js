...
while(num1 <= num2){
    ...
    if(num1%2 !=0){
        document.write('<h2>el numero '+num1+' es impar</h2>');
    }
    num1++;
}

import { browser, by, Browser, element, Button } from 'protractor';
...
describe('Prueba configuraciones generales', () => {
    ...
    //     page.getParagraphConfiguracionGeneral().click();
    //     browser.sleep(1000);
    //     expect(page.getParagraphFacturaVentaConfiguracionGeneraliiii().getAttribute('checked')).not.toBeTruthy();
    ...
    // });
    // TODO:PRECIOS
    it('deberia La opción Calculo de Precios traer por defecto Lista de Precios', () => {
        page.getParagraphConfiguracionPrecioDescuentos().click();
        // browser.sleep(5000);
        // expect(page.getParagraphFechasInventariosConfiguracionGeneral().getText()).toEqual(' Lista de Precios ');
    }); 
    // it('deberia La opción Calculo de Precios traer las opciones de Lista de Precios y Último Precio Facturado', () => {
    //     browser.actions().mouseMove(page.getParagraphFechasInventariosConfiguracionGeneral()).click().perform();

    //     var c = page.getParagraphSelectOption().count();
    //     var elm1 = page.getParagraphSelectOption().get(0).getText();
    //     var elm2 = page.getParagraphSelectOption().get(1).getText();
    //     expect(c).toEqual(2);
    //     expect(elm1).toEqual(' Último Precio Facturado ');
    //     expect(elm2).toEqual(' Lista de Precios ');
    //     page.getParagraphSelectOption().get(0).click();
    //     page.gerParagraphCerrraVentana().get(0).click();
    // }); 
  
    it('deberia tener Todas las opciones por defecto vienen inactivas', () => { 

        for (var i = 0; i < 4; i++) {
            expect(page.getParagraphDescuentosConfiguracionGeneral(i).getAttribute('checked')).not.toBeTruthy();
        }
       
    });
    it('deberia aparecer un botón automáticamente llamado Máximo Descuento, al activar activar el check de valor maximo descuento', () => {
        browser.actions().mouseMove(page.getParagraphDescuentosConfiguracionGeneral(0)).click().perform();
        browser.sleep(3000);
        var boton = element(by.id('5201EspecificoButton'));
        expect((boton).isDisplayed()).toBeTruthy();

    });
    it('deberia aparece run pop up, al dar  clic en el boton descuento maximo', () => {
        browser.sleep(3000);
        browser.actions().mouseMove(page.getParagraphConfiguracionMaximoDescuento()).click().perform();
        browser.sleep(3000);
        var ventana = element(by.id('dialog-descuentoMaximoVendedor'));
        expect((ventana).isDisplayed()).toBeTruthy();
    });
    it('deberian aparecer los terceros de tipo vendedro en la oclumna vendedor', () => {
        page.getParagraphCerrarMensajeAlerta().click();
        page.getParagraphMenuEmpresasYPersonas().click();
        page.getParagraphEmpresasYPersonasCreacionYAdminstracionTerceros().click();
        page.BurbujaFiltro();
        browser.actions().mouseMove(page.getParagraphVentanaFiltroColumna().first()).click().perform();
        browser.waitForAngularEnabled(true);
        page.getParagraphSelectOpcion('app-listado-tercero', 'inputColumna-0').get(5).click();
        browser.waitForAngularEnabled(true);
        browser.actions().mouseMove(page.getParagraphVentanaFiltroFiltro().first()).click().perform();
        browser.waitForAngularEnabled(true);
        page.getParagraphSelectOpcion('app-listado-tercero', 'inputFiltro').get(1).click();
        browser.waitForAngularEnabled(true);
        browser.actions().mouseMove(page.getParagraphVentanaFiltroCodigo().first()).click().sendKeys('Vendedor').perform();
        page.getParagraphVentanaFiltroFiltrar().click();
        browser.waitForAngularEnabled(true);
        var Vendedor = page.getParagraphInformesContarFilas('woTabla-WT_TERCEROS');
        page.getParagraphCerrarFiltrosBarraSuperior().click();
        page.gerParagraphCerrraVentana().get(1).click();
        browser.actions().mouseMove(page.getParagraphConfiguracionMaximoDescuento()).click().perform();
        browser.actions().mouseMove( page.getParagraphVendedorMaximoDesctoConfiguracionGeneral().get(0)).click().perform();
        expect(select.count()).toEqual(Vendedor);
    });
    it('', () => {
